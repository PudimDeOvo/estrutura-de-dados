class ArrayBinaryTree:
    ...
    # Additional methods like Postorder, Inorder e Preorder Traversal as asked in question 3

    def inorder(self):
        def _inorder_subtree(node):
            if node is not None:
                yield from _inorder_subtree(node.left)
                yield node.element
                yield from _inorder_subtree(node.right)
        yield from _inorder_subtree(self._root)

    def postorder(self):
        def _postorder_subtree(node):
            if node is not None:
                yield from _postorder_subtree(node.left)
                yield from _postorder_subtree(node.right)
                yield node.element
        yield from _postorder_subtree(self._root)

    def preorder(self):
        def _preorder_subtree(node):
            if node is not None:
                yield node.element
                yield from _preorder_subtree(node.left)
                yield from _preorder_subtree(node.right)
        yield from _preorder_subtree(self._root)